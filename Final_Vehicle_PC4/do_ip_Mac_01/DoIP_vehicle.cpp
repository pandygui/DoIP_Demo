...
bool createUdpSocket()
{
    ...
    int ifidx = if_nametoindex(vehicleIfname);
    if (setsockopt(vehicleIdentificationSfd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifidx, sizeof(ifidx))) {
        printf("VehicleDoIP::createUdpSocket, setsockopt IPV6_MULTICAST_IF failed!\n");
        close(vehicleIdentificationSfd);
        return false;
    }
