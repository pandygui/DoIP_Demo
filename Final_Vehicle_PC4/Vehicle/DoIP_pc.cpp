...
char vehicleIfname[IFNAME_MAX_LEN] = { 0 };
char pcIfname[IFNAME_MAX_LEN] = { 0 };
...
bool createUdpSocket()
{
    ...
    int ifidx = if_nametoindex(pcIfname);
    if (setsockopt(sfdVehicleIdentification, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifidx, sizeof(ifidx))) {
        printf("PCDoIP::createUdpSocket, setsockopt IPV6_MULTICAST_IF failed!\n");
...
bool createMulticastSocket(const char* multicastIp, uint16_t multicastPort)
{
    ...
    int ifidx = if_nametoindex(pcIfname);
    if (setsockopt(multicastSfd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifidx, sizeof(ifidx))) {
        printf("PCDoIP::createMulticastSocket, setsockopt IPV6_MULTICAST_IF failed!\n");
        close(multicastSfd);
        return false;
    }
...
bool readIfConfigFile()
{
    ...
    while (NULL != fgets(lineBuf, sizeof(lineBuf) -1, fp)) {
        ...
        else if (strncmp(tmp, VEHICLE_INTERFACE_NAME, strlen(VEHICLE_INTERFACE_NAME)) == 0) {
            tmp = tmp + strlen(VEHICLE_INTERFACE_NAME) + strlen("=");
            sscanf(tmp, "%16s", vehicleIfname);
            printf("VehicleDoIP::readIpConfigFile, Vehicle IF Name [%s]\n", vehicleIfname);
            configNum++;
            if (4 == configNum) {
                return true;
            }
        ...
        else if (strncmp(tmp, PC_INTERFACE_NAME, strlen(PC_INTERFACE_NAME)) == 0) {
            tmp = tmp + strlen(PC_INTERFACE_NAME) + strlen("=");
            sscanf(tmp, "%16s", pcIfname);
            printf("VehicleDoIP::readIpConfigFile, PC IF Name [%s]\n", pcIfname);
            configNum++;
            if (4 == configNum) {
                return true;
            }
...
{
    MT::show2(info, header);
    MT::show22(info, header);
...
{
    MT::show3(sendInfo, header, payload);
...
void announcementVehicleIdentificationDisplay(const struct SendInfoDisplay& sendInfo,
    const struct HeaderDisplay& header, const struct VehicleIdentificationDisplay& payload)
{
    MT::show4(sendInfo, header, payload);
...
{
    MT::show5(sendInfo, header, payload);
...
{
    MT::show6(sendInfo, header, payload);
...
{
    MT::show7(sendInfo, header, payload);
...
{
    MT::show9(sendInfo, header, payload);
...
{
    MT::show10(sendInfo, header, payload);
